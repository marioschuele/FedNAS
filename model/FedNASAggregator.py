...
class FedNASAggregator(object):
    def __init__(self, train_global, test_global, all_train_data_num, client_num, device, args):
        ...
        self.train_recall_dict = dict()
        self.train_acc_avg = 0.0
        self.test_acc_avg = 0.0
        self.test_loss_avg = 0.0
        self.train_recall_avg = 0.0
        self.test_recall_avg = 0.0
        ...
        self.best_recall = 0
        self.best_recall_different_cnn_counts = dict()
    ...
    def add_local_trained_result(self, index, model_params, arch_params, sample_num, train_acc, train_loss, train_recall):
        ...
        self.train_recall_dict[index] = train_recall
    ...
    def statistics(self, round_idx):
        ...
        # train recall
        train_recall_list = self.train_recall_dict.values()
        train_recall_avg = sum(train_recall_list) / len(train_recall_list)
        logging.info('Round {:3d}, Average Train Recall {:.3f}'.format(round_idx, train_recall_avg))
        wandb.log({"Train Recall": train_recall_avg, "Round": round_idx})
        ...
        # test acc
        logging.info('Round {:3d}, Average Validation Recall {:.3f}'.format(round_idx, self.test_recall_avg))
        wandb.log({"Validation Recall": self.test_recall_avg, "Round": round_idx})

        logging.info("search_train_valid_recall_gap %f" % (self.train_recall_avg - self.test_recall_avg))
        wandb.log({"search_train_valid_recall_gap": self.train_recall_avg - self.test_recall_avg, "Round": round_idx})
    ...
    def record_model_global_architecture(self, round_idx):
        ...
        wandb.log({"searching_cnn_count(%s)" % cnn_count: self.test_recall_avg, "epoch": round_idx})
        if cnn_count not in self.best_recall_different_cnn_counts.keys():
            self.best_recall_different_cnn_counts[cnn_count] = self.test_recall_avg
            summary_key_cnn_structure = "best_recall_for_cnn_structure(n:%d,r:%d)" % (
                normal_cnn_count, reduce_cnn_count)
            wandb.run.summary[summary_key_cnn_structure] = self.test_recall_avg
            ...
            if self.test_recall_avg > self.best_recall_different_cnn_counts[cnn_count]:
                self.best_recall_different_cnn_counts[cnn_count] = self.test_recall_avg
                summary_key_cnn_structure = "best_recall_for_cnn_structure(n:%d,r:%d)" % (
                    normal_cnn_count, reduce_cnn_count)
                wandb.run.summary[summary_key_cnn_structure] = self.test_recall_avg
        ...
        if self.test_recall_avg > self.best_recall:
            self.best_recall = self.test_recall_avg
            wandb.run.summary["best_valid_recall"] = self.best_recall
            wandb.run.summary["epoch_of_best_recall"] = round_idx